 ...
 *  public void setDataSource(DataSource dataSource) {
    ...
    //queryForInt() 메소드 대체 -> queryForObject(쿼리, Integer.class)
    public int getCount() {
    ...
    public void update(User user) {
        this.jdbcTemplate.update(
                "update users set name = ?, password = ?, email = ?, level = ?, login = ?, " +
                        "recommend = ? where id = ? ", user.getName(), user.getPassword(), user.getEmail(), user.getLevel().intValue(), user.getLogin(), user.getRecommend(),
                user.getId());
    }
}